...
if (isset($_GET['action']) && $_GET['action'] == 'download_report') {
    ...
    header('Content-Disposition: attachment; filename="jobseekers_report_'.date('Y-m-d').'.html"');
    
    // جلب البيانات
    $sql = "SELECT * FROM users WHERE role_id = 1 ORDER BY createdat DESC";
    ...
    echo '<div class="stats">
            <div class="stat-box">
                <div class="stat-number">'.$total_jobseekers.'</div>
                <div>إجمالي الباحثين</div>
            </div>
            <div class="stat-box">
                <div class="stat-number">'.$total_companies.'</div>
                <div>الشركات المسجلة</div>
            </div>
            <div class="stat-box">
    ...
    while($row = $result->fetch_assoc()) {
        $education = $conn->query("SELECT name FROM education WHERE id = '{$row['education_id']}'")->fetch_row()[0] ?? 'غير محدد';
...
<body>
    <div class="report-container">
        <div class="report-header">
            <h1>👤 تقرير الباحثين عن عمل</h1>
            <p>نظام بوابة الوظائف - قاعدة بيانات الباحثين</p>
        </div>
        ...
        <div class="controls">
            ...
            <div class="stats">
                <div class="stat-box"><div class="stat-number"><?php echo $total_jobseekers; ?></div><div>إجمالي الباحثين</div></div>
                <div class="stat-box"><div class="stat-number"><?php echo $total_companies; ?></div><div>الشركات المسجلة</div></div>
                <div class="stat-box"><div class="stat-number"><?php echo $total_jobs; ?></div><div>الوظائف المتاحة</div></div>
            </div>
        </div>
        ...
        <div class="table-container">
            <table>
                <thead>
                    <tr>
                        <th>#</th><th>الاسم الكامل</th><th>البريد الإلكتروني</th><th>رقم الهاتف</th><th>المؤهل العلمي</th><th>المدينة</th><th>النوع</th><th>تاريخ التسجيل</th>
                    </tr>
                </thead>
                <tbody>
                    <?php
