...
<?php include "./includes/indexHeaderr.php"; ?>
...
<body>
  ...
  <?php if (isset($_GET['key']) && isset($_GET['id'])) :
    $id_company = intval($_GET['id']); // تأمين المتغير
    ...
    //getting state info
    $division_or_state_id = intval($row['state_id']);
    $sql1 =  "SELECT * FROM states WHERE id = ?";
    $stmt1 = $conn->prepare($sql1);
    $stmt1->bind_param("i", $division_or_state_id);
    $stmt1->execute();
    $query1 = $stmt1->get_result();
    $row1 = $query1->fetch_assoc();

    //getting city info
    $district_or_city_id = intval($row['city_id']);
    $sql2 =  "SELECT * FROM districts_or_cities WHERE id = ?";
    $stmt2 = $conn->prepare($sql2);
    $stmt2->bind_param("i", $district_or_city_id);
    $stmt2->execute();
    ...
    $sql3 =  "SELECT * FROM industry WHERE id = ?";
    $stmt3 = $conn->prepare($sql3);
    $stmt3->bind_param("i", $industry_id);
    $stmt3->execute();
    $query3 = $stmt3->get_result();
    $row3 = $query3->fetch_assoc();

    // getting reviews info
    $sql5 = "SELECT * FROM company_reviews where company_id = ?";
    $stmt5 = $conn->prepare($sql5);
    $stmt5->bind_param("i", $id_company);
    $stmt5->execute();
    $query5  = $stmt5->get_result();
              ...
              <div class="banner-headline-text-part">
                ...
                <div class="job-info-container">
                  ...
                  <div class="job-category-info">
                    ...
                    <span><?php echo $row3['name'] . " صناعة" ?></span>
                  </div>
        ...
        <div class="company-details-page-content-left-side">
          ...
          <div class="review-container-area">
            <div class="headline">
              ...
              <h3>مراجعة الشركة - تمت المراجعة (<?php echo $query5->num_rows; ?>)</h3>
            </div>
                ...
                <div class="star-widget">
                  <form action="./process/submitReview.php?key=<?php echo md5($id_company) . "&cid=" . $id_company  ?>" method="post">
                    ...
                    <div class="review-form">
                      <header></header>
                      <div class="textarea">
                        <textarea name="company-review" cols="30" placeholder="صف تجربتك.."></textarea>
                      </div>
                      <div class="button-container">
                        <button class="btn" type="submit" name="submit">إرسال المراجعة</button>
                      </div>
                    </div>
                  </form>
                </div>
